...
export default function Overview() {
  ...
  // Notificações push
  useEffect(() => {
    if (nextExam && 'Notification' in window && Notification.permission === 'granted') {
      const examDate = new Date(nextExam.data_prova);
      const daysUntilExam = Math.ceil((examDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));

      if (daysUntilExam === 3) {
        new Notification('Prova próxima!', {
          body: `Sua prova de ${nextExam.materia} está em 3 dias.`,
          icon: '/icon.svg',
          badge: '/icon.svg'
        });
      }
          ...
          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-green-400 font-medium">Instale a LexIA no seu dispositivo</p>
                  <p className="text-green-300 text-sm">Acesse mais rápido e offline</p>
                </div>
              </div>
              <button
